...
func buildRoomManagerReviewPrompt(task Task) string {
	...
	b.WriteString("- **Semantic alignment required** (text only, do not inspect workspace):\n")
	b.WriteString("  1. Find the user's **original ask** in context (user_root / quote chain).\n")
	b.WriteString("  2. Classify: **do-for-me** (open, run, create, fix, deploy…) vs **info** (where, how, what).\n")
	b.WriteString("  3. Compare agent reply semantics:\n")
	b.WriteString("     - do-for-me + agent only gives **instructions/paths for the user to act** → **NOT met** → `relay_to` / `route_to` same or best agent; relay_reason must state the gap (e.g. user asked to open file, agent only explained how).\n")
	b.WriteString("     - do-for-me + agent clearly states done/executed/delivered → OK for `notify_user`.\n")
	b.WriteString("     - info ask + matching info in reply → OK for `notify_user`.\n")
	b.WriteString("- **Never notify_user** when user said \"help me open/run/fix\" and agent only said \"double-click / open in browser / follow these steps\".\n")
	b.WriteString("- Do **NOT** verify files/paths yourself — but if dialogue shows unmet do-for-me intent, **relay**, do not close.\n")
	b.WriteString("- Ambiguous → `ask_user`; still open → `route_to` / `relay_to`; next agent → `relay_to` with `relay_reason`.\n\n")
	appendRoomManagerContext(&b, task)
	appendRoomManagerFooter(&b)
	return b.String()
}
