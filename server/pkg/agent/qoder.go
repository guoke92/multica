...
// overridden by user-configured custom_args. Qoder CLI enters ACP server
// mode via the root `--acp` flag (not an `acp` subcommand — see
// https://docs.qoder.com/zh/cli/acp). `--yolo` enables Bypass Permissions
// mode so headless daemon runs do not stall on tool approval prompts.
var qoderBlockedArgs = map[string]blockedArgMode{
	"--acp": blockedStandalone,
	"--yolo": blockedStandalone,
}

// qoderBackend implements Backend by spawning `qoderclicn --acp --yolo`
// (or `qodercli` when that is the resolved binary) and communicating via
// the ACP JSON-RPC 2.0 transport over stdin/stdout — the same protocol
// stack as Hermes / Kimi / Kiro.
