...
/** Debounced graph refetch — coalesces burst WS events without clearing cached snapshot. */
...
/** Immediate graph refetch when incremental cache cannot represent new entities. */
...
export function mergeRoomGraphSnapshot(
  old: RoomGraphSnapshot | undefined,
  patch: Partial<RoomGraphSnapshot>,
): RoomGraphSnapshot | undefined {
  if (!old) return old;
  return { ...old, ...patch };
}
...
/** Append a confirmed message to the infinite messages cache (newest page tail). */
export function appendRoomMessageToInfiniteCache(
  qc: QueryClient,
  wsId: string,
  roomId: string,
  message: RoomMessage,
  ...
  qc.setQueryData<{ pages: RoomMessage[][]; pageParams: unknown[] } | undefined>(
    key,
    (old) => {
...
/** Replace optimistic user message with the server-confirmed row. */
      ...
      const pages = old.pages.map((page, i) => {
        ...
        if (optimisticIdx >= 0) {
          ...
          next[optimisticIdx] = {
            ...
            // Keep optimistic identity fields when the send response omits them.
...
export function patchRoomGraphCache(
  qc: QueryClient,
  wsId: string,
  roomId: string,
