...
func TestBuildCommentReplyInstructionsNonCodexLinux(t *testing.T) {
	...
	for _, host := range []string{"linux", "darwin"} {
		for _, provider := range []string{"claude", "opencode", "openclaw", "hermes", "kimi", "kiro", "qoder", "cursor", "gemini"} {
			name := provider + "/" + host
...
func TestBuildCommentReplyInstructionsWindowsUsesContentFile(t *testing.T) {
	...
	for _, provider := range []string{"codex", "claude", "opencode", "openclaw", "hermes", "kimi", "kiro", "qoder", "cursor", "gemini"} {
